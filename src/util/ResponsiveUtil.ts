...
const onOrientationChangeListener = (orientationChangeEvent: OrientationChangeEvent
) => {
  calculateDimensions();
}

export function initiateListenrs() {
  const orientationListener = ScreenOrientation.addOrientationChangeListener(onOrientationChangeListener);
