...
const StatusBarContext = createContext<StatusBarContextValue | null>(
  null
) as Context<StatusBarContextValue>
...
export const useStatusBarContext = (): StatusBarContextValue => {
  ...
  if (!ctx)
    throw new Error(
      'StatusBarContext has been used outside StatusBarContext.Provider'
    )
...
export const DeviceLayerContext = ({ children }) => {
  ...
  return (
    <StatusBarContext.Provider value={[state, dispatch]}>
      <StatusBar animated={true} translucent={false} hidden={state.hidden} />
      {children}
